...
void main() {
  group('MovieDetailBloc', () {
    ...
    blocTest<MovieDetailBloc, MovieDetailState>(
      ...
      build: () {
        when(() => repository.getMovieDetail(movieCd: '1', forceRefresh: false))
            .thenAnswer((_) async => Result.ok(detail));
        return MovieDetailBloc(movieDetailRepository: repository);
      },
      act: (bloc) => bloc.add(MovieDetailFetched(movieCd: '1')),
      expect: () => [
        MovieDetailState(status: MovieDetailStatus.success, movieDetail: detail),
      ],
    );
